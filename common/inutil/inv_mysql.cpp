...
INV_Mysql::INV_Mysql(const string& sHost, const string& sUser, const string& sPasswd, const string& sDatabase, const string &sCharSet, int port, int iFlag)
:_bConnected(false)
{
...
INV_Mysql::INV_Mysql(const INV_DBConf& tcDBConf)
:_bConnected(false)
{
    _dbConf = tcDBConf;
...
void INV_Mysql::init(const string& sHost, const string& sUser, const string& sPasswd, const string& sDatabase, const string &sCharSet, int port, int iFlag)
{
    _dbConf._host = sHost;
    _dbConf._user = sUser;
...
void INV_Mysql::init(const INV_DBConf& tcDBConf)
{
    _dbConf = tcDBConf;
}

void INV_Mysql::connect()
{
    disconnect();
    ...
    if(!_dbConf._charset.empty())  {
        if (mysql_options(_pstMql, MYSQL_SET_CHARSET_NAME, _dbConf._charset.c_str())) {
            throw INV_Mysql_Exception(string("INV_Mysql::connect: mysql_options MYSQL_SET_CHARSET_NAME ") + _dbConf._charset + ":" + string(mysql_error(_pstMql)));
        }
    }
    ...
    {
        throw INV_Mysql_Exception("[INV_Mysql::connect]: mysql_real_connect: " + string(mysql_error(_pstMql)));
    }
    
    _bConnected = true;
}

void INV_Mysql::disconnect()
{
    if (_pstMql != NULL)
    {
        mysql_close(_pstMql);
...
string INV_Mysql::escapeString(const string& sFrom)
{
    if(!_bConnected)
    {
        connect();
...
MYSQL *INV_Mysql::getMysql(void)
{
    return _pstMql;
}

string INV_Mysql::buildInsertSQL(const string &sTableName, const RECORD_DATA &mpColumns)
{
...
string INV_Mysql::buildReplaceSQL(const string &sTableName, const RECORD_DATA &mpColumns)
{
    ostringstream sColumnNames;
    ostringstream sColumnValues;
...
string INV_Mysql::buildUpdateSQL(const string &sTableName,const RECORD_DATA &mpColumns, const string &sWhereFilter)
{
...
string INV_Mysql::getVariables(const string &sName)
{
...
void INV_Mysql::execute(const string& sSql)
{
    if(!_bConnected)
    {
        connect();
    ...
    if (iRet != 0)
    {
        throw INV_Mysql_Exception("[INV_Mysql::execute]: mysql_query: [ " + sSql+" ] :" + string(mysql_error(_pstMql)));  
    }
}

INV_Mysql::MysqlData INV_Mysql::queryRecord(const string& sSql)
{
    MysqlData   data;
    ...
    if(pstRes == NULL)
    {
        throw INV_Mysql_Exception("[INV_Mysql::queryRecord]: mysql_store_result: " + sSql + " : " + string(mysql_error(_pstMql)));
    }
...
size_t INV_Mysql::updateRecord(const string &sTableName, const RECORD_DATA &mpColumns, const string &sCondition)
{
...
size_t INV_Mysql::insertRecord(const string &sTableName, const RECORD_DATA &mpColumns)
{
...
size_t INV_Mysql::replaceRecord(const string &sTableName, const RECORD_DATA &mpColumns)
{
...
size_t INV_Mysql::deleteRecord(const string &sTableName, const string &sCondition)
{
    ostringstream sSql;
...
size_t INV_Mysql::getRecordCount(const string& sTableName, const string &sCondition)
{
    ostringstream sSql;
...
size_t INV_Mysql::getSqlCount(const string &sCondition)
{
    ostringstream sSql;
...
int INV_Mysql::getMaxValue(const string& sTableName, const string& sFieldName,const string &sCondition)
{
    ostringstream sSql;
...
bool INV_Mysql::existRecord(const string& sql)
{
    return queryRecord(sql).size() > 0;
}

long INV_Mysql::lastInsertID()
{
    return mysql_insert_id(_pstMql);
}

size_t INV_Mysql::getAffectedRows()
{
    return mysql_affected_rows(_pstMql);
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////
INV_Mysql::MysqlRecord::MysqlRecord(const map<string, string> &record)
: _record(record)
{
}

const string& INV_Mysql::MysqlRecord::operator[](const string &s)
{
    map<string, string>::const_iterator it = _record.find(s);
    if(it == _record.end())
    {
        throw INV_Mysql_Exception("field '" + s + "' not exists.");
    }
    return it->second;
}
...
vector<map<string, string> >& INV_Mysql::MysqlData::data()
{
    return _data;
}

size_t INV_Mysql::MysqlData::size()
{
    return _data.size();
}

INV_Mysql::MysqlRecord INV_Mysql::MysqlData::operator[](size_t i)
{
